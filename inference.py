...
def score_yes_no_batch(model, tokenizer, prompts, yes_token="yes", no_token="no"):
    """
    각 prompt에 대해 'yes'와 'no' 후보의 총 로그확률을 계산하고
    softmax로 확률을 만들어 반환.
    반환: list of dicts: {"logprob_yes": float, "logprob_no": float, "prob1": float}
    """
    results = []
    device = next(model.parameters()).device

    # 토큰화를 빠르게 하기 위해 한번에 처리
    ...
    # logits: (B, T, V). 후보 영역만 선택해 토큰별 NLL 계산
    ...
    # 위에서는 배치 전체 합산이므로, 배치 내 샘플별 로그확률이 필요하면
    # 샘플wise로 계산하는 버전을 아래처럼 간단 루프 구현 가능.
    # (속도가 아주 중요하지 않다면 샘플별 루프가 이해하기 쉬움)
    results = []
    for b in range(len(prompts)):
        # 개별 샘플 스코어 재계산(간단/명확 버전)
        res = {}
...
def main(
    ...
    save_scores: bool = False,     # ← 추가: 확률 저장 온/오프
