...
def compute_metrics(p: EvalPrediction):
    ...
    # If logits is a tuple/list (e.g., logits, hidden_states), take only logits
    if isinstance(logits, (tuple, list)):
        logits = logits[0]

    # Ensure logits is a 2D array: [batch_size, num_labels]
...
def train(
    ...
    cutoff_len: int = 260, # QC data 95 + prompt 154
    val_set_size: int = 1,
    ...
    sample = train_data[0]  # or raw trn_df.iloc[0]
    print(sample["input_ids"])
    print(sample["labels"])
    print(tokenizer.decode(sample["input_ids"]))
    print(tokenizer.decode([id for id in sample["labels"] if id != -100]))
    ...
    model_size = os.path.getsize(pytorch_model_path) / (1024 * 1024) if os.path.exists(pytorch_model_path) else None
    ...
    if val_set_size > 0:
        ...
        metrics = trainer.evaluate(eval_dataset=train_data)
